...
import {CONSTRUCT_TARGET_POOL_ID} from "../constants";
...
export class ConstructionSiteCreatedEventHandler extends EventHandler<any> {
  public handle(eventEntry: ConstructionSiteCreatedEvent): boolean {
    ...
    const sites = room.lookForAt("constructionSite", eventEntry.x, eventEntry.y)
      .filter(site => site.structureType === eventEntry.buildingType);
    if (sites.length === 0) {
      return true;
    }

    Globals.getGlobal<TargetPool<any, any>>(TargetPool as any, getIdFromRoom(room, CONSTRUCT_TARGET_POOL_ID))
      ?.addTarget(sites[0]);
